...
`docs/decisions.md` DEC-005 records an open question about whether a task owns exactly "one logical task record"; do not treat that claim as settled until DEC-005 is resolved.
...
`docs/docs/research/tasks/_index.md` § "Common data architecture" defines task composition of multiple independent substrates and why that composition remains task build context rather than a new substrate.
Consult it rather than re-deriving the claim here.

## Categorical identity

`docs/docs/research/tasks/_index.md` § "Directed semantic graph" and `docs/invariants.md` DATA-002 establish that Dagflow node IDs are not observation IDs absent an explicit task-owned binding.

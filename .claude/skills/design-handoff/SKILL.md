...
- The target file is `.claude/handoffs/<identifier>.md`.
...
1. Apply the readiness test in `.claude/handoffs/README.md` § "When a handoff may be written".
2. If a material decision remains unresolved, write nothing.
   ...
   Stop there.
3. If ready, persist every durable semantic decision in its correct authoritative location first — `docs/authority.md`, `docs/invariants.md`, or the owning specification — per `CLAUDE.md`'s specification-first and synchronization rules.
   Handoff files are untracked and unrecoverable from git, so this ordering is what prevents design loss. The handoff carries only the implementation-facing residue.
4. Then write the handoff file per `.claude/handoffs/README.md`, with `status: agreed` frontmatter.
...
This skill closes the design phase. It does not implement, and it does not enter the implementation phase — `.claude/handoffs/README.md` § "Flow" describes how that phase is entered.

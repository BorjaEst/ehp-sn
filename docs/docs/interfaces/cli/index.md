...
| Command                   | Purpose                                                         | Component maturity |
| ------------------------- | --------------------------------------------------------------- | ------------------ |
...
| [`report`](report.md)     | Package existing results for presentation or export             | Specified          |

`Component maturity` reflects the component as a whole (specification, implementation, and validation evidence together), following `planned → specified → implemented → validated → reference`. Every command page's own `document_status` is `specified` and `capability_status` is `planned` (spec complete, no implementation yet).
...
`validate` checks configuration, compatibility, resource availability, and destination constraints at a requested level whose exact vocabulary is domain-specific — for example, `train validate` uses `config`, `resources`, or `build`; `data validate` uses `quick` or `full`; `tasks validate` uses `manifest`, `sample`, or `full`.
It does not execute the operation.
Validation failure exits with code `5`.
...
package and schema defaults
    ...
    < --config file
    < --set overrides and dedicated invocation options (same precedence level)
```

A dedicated invocation option such as `--device` replaces the corresponding value from `--config` or workspace defaults because its meaning is explicitly owned by that command.
Supplying both a dedicated option and a `--set` override for the same field is not resolved by precedence: it is rejected as an ambiguous duplicate explicit input.
See [Configuration resolution](../configuration/resolution.md#invocation-explicit-conflict-rule).
...
| `--workspace PATH`    | None                         | Operations that resolve an optional explicit workspace configuration |
...
experiment:mazehard-hrm/v1
experiment:routebind-hrm/v1
```
...
9. Logical references may be resolved by local or configured persistence adapters. Their manifests or artifact fingerprints, not machine-specific physical paths, determine identity.
